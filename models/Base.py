...
class BaseModel(torch.nn.Module):
    ...
    def encode_input(self, X, y, cat_features, normalize_features, replace_na,
    ...
    def train_model(self, model_in, target_labels, train_mask, optimizer):
        ...
        if self.task == 'regression':
            loss = torch.sqrt(F.mse_loss(pred.squeeze(), y))
        elif self.task == 'classification':
            loss = F.cross_entropy(pred, y.long())
        else:
            raise NotImplemented("Unknown task. Supported tasks: classification, regression.")

        if self.logit_t is not None:
            dl = self.distill_loss(pred, self.logit_t[train_mask])
            # print('XEnt loss', loss)
            # print('Distill loss', dl)
            # print()
            loss = loss + dl
    ...
    def evaluate_model(self, logits, target_labels, mask):
        ...
        with torch.no_grad():
            ...
            if self.task == 'regression':
                ...
                if self.logit_t is not None:
                    metrics['dl_loss'] = self.distill_loss(pred, self.logit_t[mask])
    ...
    def plot_interactive(self, metrics, legend, title, logx=False, logy=False, metric_name='loss', start_from=0):
        ...
        if isinstance(metric_name, str):
            ...
            metric_results = [metrics[metric] for metric in metric_name]
            metrics = metric_name

        for ix_metric, res in enumerate(metric_results):
            xs = [list(range(len(res)))] * len(res[0])
            ys = list(zip(*res))
